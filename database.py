...
# ========== НАСТРОЙКА ПУТИ К БАЗЕ ДАННЫХ (ДЛЯ KOYEB) ==========
# Koyeb позволяет писать в /tmp, но данные не сохраняются между перезапусками
# Для постоянного хранения используй volume или перейди на PostgreSQL
DB_DIR = "/tmp/data"
...
async def seed_data():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if count == 0:
            ...
            # Университеты (фото пока пустые - добавишь позже)
            universities = [
